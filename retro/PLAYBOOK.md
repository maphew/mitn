...
**Amp** — threads are server-resident on current installations. Inventory with
`amp threads list --include-archived --json` and fetch an exact thread with
`amp threads export <id>`. A matching file under
`~/.local/share/amp/threads/` is only an offline fallback for that exact id,
